...
uintptr_t FindPattern(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, char* szMask, uintptr_t offset = 0)
{
	for (uintptr_t i = 0; i < dwLen; i++)
	{
		if (IsBadReadPtr((void*)(dwAddress + i), 4) == 0 && DataCompare((BYTE*)(dwAddress + i), bMask, szMask))
		{
			return (uintptr_t)(dwAddress + i + offset);
		}
	}
...
uintptr_t CMemory::Pattern(std::shared_ptr<PatternStruct> Struct)
{
	...
	uintptr_t address = FindPattern((uintptr_t)rangeStart, dwFileSize, (BYTE*)ret.first.data(),(char*) ret.second.data(), Struct->offset);

	if (!address)
		return 0;

	if (ret.first.at(0) == 0xE8) { //is a call function
		uintptr_t relFuncAddr = *(int*)(address + 1);
	...
	{
		hde64s hd;
		const uint32_t size = hde64_disasm(reinterpret_cast<const void*>(address - Struct->offset),&hd);
		address = ((address - Struct->offset) + size) + *(int*)((address));
		address -= (uintptr_t)rangeStart;
	}
